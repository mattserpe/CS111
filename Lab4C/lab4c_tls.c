...
void report(int shutdown){
	...
	if(shutdown || (genReports && (prevTime == 0 || clk - prevTime >= period))){
		...
		if(shutdown || genReports){
			...
			if(SSL_write(sslClient, report, strlen(report)) < 0){
				fprintf(stderr, "Error writing over SSL\n");
				exit(2);
			}
		}
...
void polling(){
	...
	while(1){
		...
		if(x && input.revents == POLLIN){
			...
			int numCharSocketIn = SSL_read(sslClient, &socketInput, 2048);
...
void ssl(){
	...
	if(sslClient == NULL) {
		fprintf(stderr, "Error in SSL setup\n");
		exit(2);
	}
...
int main(int argc, char* argv[]){
	...
	char ID[16];
	sprintf(ID, "ID=%s\n", id);
	if(SSL_write(sslClient, ID, strlen(ID)) < 0){
		fprintf(stderr, "Error writing over SSL\n");
		exit(2);
	}
	dprintf(fd, "ID=%s\n", id);
